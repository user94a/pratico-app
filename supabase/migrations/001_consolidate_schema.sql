...
CREATE POLICY "documents_update_own" ON public.documents
  FOR UPDATE USING (user_id = auth.uid() AND user_id = (SELECT user_id FROM public.documents WHERE id = OLD.id));

DROP POLICY IF EXISTS "documents_delete_own" ON public.documents;
CREATE POLICY "documents_delete_own" ON public.documents
  FOR DELETE USING (user_id = auth.uid() AND user_id = (SELECT user_id FROM public.documents WHERE id = OLD.id));
