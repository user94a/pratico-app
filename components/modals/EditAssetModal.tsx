...
export function EditAssetModal({ visible, onClose, asset, onUpdate }: {
  visible: boolean;
  onClose: () => void;
  asset: Asset | null;
  onUpdate: () => void;
}) {
  const [type, setType] = useState<typeof ASSET_TYPES[number]>(asset?.type as any || 'vehicles');
  const [name, setName] = useState(asset?.name || '');
  const [identifier, setIdentifier] = useState(asset?.identifier || '');
  const [customIcon, setCustomIcon] = useState<string>(asset?.custom_icon || '');
  const [showIconPicker, setShowIconPicker] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  // Aggiorna i valori quando cambia l'asset
  React.useEffect(() => {
    if (asset) {
      setType(asset.type as any);
      setName(asset.name);
      setIdentifier(asset.identifier || '');
      setCustomIcon(asset.custom_icon || '');
    }
  }, [asset]);

  const handleSubmit = async () => {
    if (!asset || !name.trim()) {
      Alert.alert('Errore', 'Il nome è richiesto');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Successo', 'Bene aggiornato');
      onUpdate();
      onClose();
    } catch (error: any) {
      Alert.alert('Errore', error.message || 'Impossibile aggiornare il bene');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }}>
          <View style={{ flex: 1, padding: 16 }}>
            <Text style={{ fontSize: 28, fontWeight: '800', marginBottom: 24 }}>Modifica bene</Text>
